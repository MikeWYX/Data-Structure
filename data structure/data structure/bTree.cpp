...
void destroyvalue(bNode *root,int value){
	bNode *temp1,*temp2;
	while((temp1=locateByvalue(root,value))){
...
bNode *sharedparent(bNode *root,int id1,int id2){
	int depth1,depth2;
	bNode *temp;
	temp=locateByID(root,id1);
	while(!locateByID(temp,id2)){
		temp=parent(root,temp);
	}
	return temp;  
}

int maxvalue(bNode *root,int max){
	int temp;
	if(!root){
		return max;
	}
	...
	if((temp=maxvalue(root->lchild,max))>max){
		max=temp;
	}
	if((temp=maxvalue(root->rchild,max))>max){
		max=temp;
	}
	return max;
}

int minvalue(bNode *root,int min){
	int temp;
	if(!root){
		return min;
	}
	...
	if((temp=minvalue(root->lchild,min))<min){
		min=temp;
	}
	if((temp=minvalue(root->rchild,min))<min){
		min=temp;
	}
	return min;
}

int maxdeviation(bNode *root){
	int max=-99999,min=99999;
	max=maxvalue(root,max);
	min=minvalue(root,min);
	return (max-min);
}

void printpath(bNode *root,int id){
	bNode *temp1,*temp2;
	temp2=locateByID(root,id);
	temp1=parent(root,temp2);
	if(temp2==root){
		return;
	}
	printpath(root,temp1->data.id);
	if(temp1->lchild==temp2){
		printf("左");
	}
	if(temp1->rchild==temp2){
		printf("右");
	}
}
